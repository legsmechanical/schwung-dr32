...
export const CORE_LIBRARY = '/data/CoreLibrary';

/** Sample URI roots. `ableton:/packs/abl-core-library` is the factory Core
 *  Library (the stock benchmark kits use it) and lives OUTSIDE the user
 *  library — a resolver that only knows user-library silently drops every
 *  factory sample. Located on device 2026-07-25. */
const URI_ROOTS = [
...
/** Chain-mixer pan is serialized in the -50..+50 domain, NOT -1..+1.
 *  Verified against the device corpus (real presets carry values like -32.812
 *  and +31.125) and the reconstructed equal-power law. */
...
export function resolveUri(uri, roots = URI_ROOTS) {
  if (!uri) return null;
...
export function parseKit(jsonText) {
  ...
  const pads = (rack.chains || []).map((chain, index) => {
    ...
    return {
      ...
      mixer: {
        volume: mixer.volume ?? 0,            // dB; gain = 10^(dB/20)
        pan: mixer.pan ?? 0,                  // -50..+50 (see PAN_MIN/PAN_MAX)
        send: mixer.sends && mixer.sends[0] ? mixer.sends[0].amount : -70,
        speakerOn: mixer.speakerOn !== false, // false = sample-exact silence
      },
...
export function serializeKit(kit, { indent = 1 } = {}) {
  ...
  rack.chains = kit.pads.map((pad) => {
    ...
    if ('speakerOn' in chain.mixer) chain.mixer.speakerOn = pad.mixer.speakerOn;
