...
for src in dsp/engines/*.cpp; do
    $CXX -O3 -fPIC $ARCH -DNDEBUG -std=c++14 -Wall -Wextra -Wno-comment -Wno-unused-parameter -Wno-missing-field-initializers \
        -fno-exceptions -fno-rtti -fvisibility=hidden -Idsp -Idsp/engines \
        -c "$src" -o "build/obj/eng_$(basename "${src%.cpp}").o"
done
# The C engines (9W9 is a C11 machine and must stay one — see
# dsp/engines/9w9_engine.c). -O3 is the port's own flag; the same glob as
# tests/run.sh, for the same reason as above.
for src in dsp/engines/*.c; do
    $CC -O3 -fPIC $ARCH -DNDEBUG -std=c11 -Wall -Wextra -fvisibility=hidden -Idsp -Idsp/engines \
        -c "$src" -o "build/obj/eng_$(basename "${src%.c}").o"
done
...
cp src/chain_params.json "dist/${MODULE_ID}/"
# The synth engines' pages. dsp.so merges this into the hierarchy it serves
# (see merge_engine_ui in dsp/dr32.c); without it a synth pad shows only its
# Pad and Mix banks. Generated by tools/gen_engine_ui.mjs.
cp src/engine_ui.json "dist/${MODULE_ID}/"
