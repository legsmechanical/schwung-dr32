...
/* Index of the bracket that closes the one at s[open_at], skipping strings;
 * -1 if unbalanced. */
static long match_bracket(const char *s, long open_at) {
    char open = s[open_at], close = open == '{' ? '}' : ']';
    int depth = 0, in_str = 0;
    for (long i = open_at; s[i]; i++) {
        char c = s[i];
        if (in_str) {
            if (c == '\\' && s[i + 1]) i++;
            else if (c == '"') in_str = 0;
...
static char *read_small_file(const char *path, long *out_len) {
...
static char *merge_engine_ui(char *hier, int *len, const char *module_dir) {
    ...
    if (after && nav && lvs && hlv) {
        ...
        long nav_open = (long)(strchr(nav, '[') - eu), lvs_open = (long)(strchr(lvs, '{') - eu);
        long nav_close = match_bracket(eu, nav_open), lvs_close = match_bracket(eu, lvs_open);
        long hl_open = (long)(strchr(hlv, '{') - hier), hl_close = match_bracket(hier, hl_open);
        const char *entry = NULL;
        if (an_end && an_end - an < 60) {
            snprintf(anchor, sizeof(anchor), "{\"level\":\"%.*s\"", (int)(an_end - an), an);
            entry = strstr(hier, anchor);
        }
...
static void set_param(void *instance, const char *key, const char *val) {
    ...
    /*
     * RESAMPLE (Josh, 2026-09-22):
     *   "<header> Resample Pad / Resample kit. clicking pad pulls up a screen
     *    that displays the last tapped pad name and number on one line and
     *    velocity level on the next (and updates as you hit pads) under that
...
static int get_param(void *instance, const char *key, char *buf, int buf_len) {
     ...
     * name and the velocity it was last hit with — live, so the dialog follows
