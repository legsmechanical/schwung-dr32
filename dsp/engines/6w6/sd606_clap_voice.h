/*
 * sd606_clap_voice.h — ClapVoice with the colour bank's inner loop fixed.
 *
 * The project's second fork of vendored code, and like the first a reluctant
 * one. src/vendor/606/Clap.hpp is untouched; this is a copy with ONE function
 * changed (plus the configure() that feeds it), renamed so both can live in
 * the same binary and be A/B'd.
 *
 * WHY. An ablation on the Move put the clap's cost almost entirely in one
 * place:
 *
 *     full ClapVoice::process    7.16% of realtime
 *     ClapCore::process only     6.11%
 *     colour bank + rng only     4.73%   <-- two thirds of the whole voice
 *     the 9 envelope exp()s      1.40%
 *
 * CorrelatedColorBank runs 192 taps x 4 filters per sample and computed its
 * ring-buffer index with an INTEGER MODULO on every tap.
 *
 * WHAT CHANGED. `process` reads from a doubled history at a descending
 * pointer; `push` writes each sample at two offsets. Same taps, same values,
 * same accumulation order, so the output is BIT-IDENTICAL -- not "close", not
 * "spectrally similar": every sample is the same float. test/clap_equiv
 * asserts exactly that against the vendored class.
 *
 * RESULT, measured on the Move: the voice goes 7.62% -> 6.05% of realtime,
 * and the colour bank itself 4.6% -> 2.06%. Split of what is left, per the
 * same ablation: bank 2.06%, envelopes ~2.3%, voice-level tanh/fade ~1.5%,
 * rng 0.14%.
 *
 * The envelope exp()s are LEFT ALONE deliberately. They are the next ~1.3
 * points and a recursive multiply would get them (1.40% -> 0.10%), but that
 * means restructuring seven stateless gate helpers into stateful objects --
 * a much larger diff against upstream -- AND it would no longer be
 * bit-identical, only inaudibly close. Not worth trading a provable claim for
 * a fifth of the voice. Revisit only if the budget demands it.
 *
 * GPL-3.0. Derived from 606-Inspired-Synth-Drums (MIT, Matthew Fecher).
 */
#ifndef SD606_CLAP_VOICE_H
#define SD606_CLAP_VOICE_H
...
public:
    ...
    const float *data() const { return coefficients_.data(); }
...
public:
    void configure(float sourceStep) {
        ...
        // Interleave the four curves as {opening, handoff, fast, late} per
        // tap. That is what lets the inner loop below be one 4-wide multiply
        // per tap instead of four scalar ones -- see the comment there.
        // Built once per trigger, next to configure()'s own 4 x 192 cubic
        // resample, so it costs nothing next to what is already happening.
        for (std::size_t tap = 0; tap < tapCount_; ++tap) {
            interleaved_[tap * 4 + 0] = opening_.data()[tap];
            interleaved_[tap * 4 + 1] = handoff_.data()[tap];
            interleaved_[tap * 4 + 2] = fast_.data()[tap];
            interleaved_[tap * 4 + 3] = late_.data()[tap];
        }
        history2_.fill(0.0f);
        position_ = 0;
    }
    ...
    ColorFrame process(float noise) {
        push(noise);
        ColorFrame output;
        // Same taps, same values, same accumulation ORDER as the vendored
        // loop -- but read from a DOUBLED history, so the index is a plain
        // descending walk instead of an integer modulo on every tap.
        //
        // With history2_[i] == history2_[i + tapCount_], the vendored
        //     (position_ + tapCount_ - 1 - tap) % tapCount_
        // is exactly base - tap for base = position_ + tapCount_ - 1; that run
        // stays inside the doubled buffer, so no wrap arithmetic is needed and
        // the reads become contiguous (which also lets the compiler vectorise
        // the four dot products). 192 taps x 4 filters meant 192 integer
        // modulos per sample, paid for nothing. Output is bit-identical.
        const float *h = history2_.data() + (position_ + tapCount_ - 1);
        const float *__restrict c = interleaved_.data();
        // Four INDEPENDENT accumulators over interleaved coefficients. The
        // vectorisation here is across the four FILTERS, not across taps --
        // which matters: vectorising across taps would reassociate a float
        // sum and change the result, and -ffast-math is deliberately off.
        // Each filter keeps its own accumulation order exactly as the
        // vendored loop had it, so this stays bit-identical while collapsing
        // to one 4-wide multiply-add per tap.
        float o = 0.0f, hf = 0.0f, ft = 0.0f, lt = 0.0f;
        for (std::size_t tap = 0; tap < tapCount_; ++tap, c += 4) {
            const float sample = h[-static_cast<std::ptrdiff_t>(tap)];
            o  += c[0] * sample;
            hf += c[1] * sample;
            ft += c[2] * sample;
            lt += c[3] * sample;
...
private:
    void push(float sample) {
        const float s = flushDenormal(sample);
        // Written twice, tapCount_ apart: that is what makes the read above a
        // contiguous run with no wrap test.
        history2_[position_] = s;
        history2_[position_ + tapCount_] = s;
    ...
    std::array<float, kColorTapCount * 2> history2_ = {{}};
    std::array<float, kColorTapCount * 4> interleaved_ = {{}};
...
class Sd606ClapVoice {
public:
    void init(double sampleRate, uint32_t seed = 0x0606C1A9u) {
        sampleRate_ = std::isfinite(sampleRate)
            && sampleRate >= Sd606ClapDetail::kMinimumSupportedSampleRate
            && sampleRate <= Sd606ClapDetail::kMaximumSupportedSampleRate
            ? sampleRate : Sd606ClapDetail::kReferenceSampleRate;
        core_.init(seed);
        stop();
    }
    ...
    void trigger(float decayPercent, float pitchRatio,
        ...
        const double desiredCoreRate = Sd606ClapDetail::kReferenceSampleRate * ratio;
        ...
        const double airCutoff = std::min(
            Sd606ClapDetail::kAirHighpassHz, 0.35 * sampleRate_);
        airHighpassCoefficient_ = static_cast<float>(std::exp(
            -2.0 * Sd606ClapDetail::kClapPi * airCutoff / sampleRate_));
        airHighpassInput_ = 0.0f;
        airHighpassOutput_ = 0.0f;
        maximumSamples_ = std::max<uint32_t>(1, static_cast<uint32_t>(
            std::ceil(sampleRate_ * Sd606ClapDetail::kVoiceDurationSeconds)));
        sampleIndex_ = 0;
        active_ = true;
    }
    ...
    float process() {
        ...
        const Sd606ClapDetail::CoreFrame raw = coreStep_ == 1.0f
            ? core_.process() : upsampler_.process(core_);
        const double time = static_cast<double>(sampleIndex_) / sampleRate_;
        const double fadePosition = std::max(0.0, std::min(
            (time - 0.290)
                / (Sd606ClapDetail::kVoiceDurationSeconds - 0.290), 1.0));
        const double fadeBase = 0.5 + 0.5
            * std::cos(Sd606ClapDetail::kClapPi * fadePosition);
        const double fade = fadeBase * fadeBase * fadeBase * fadeBase;
        const double faded = static_cast<double>(raw.full) * fade;
        const double calibrated = Sd606ClapDetail::kSaturationThreshold * std::tanh(
            faded / Sd606ClapDetail::kSaturationThreshold);
        double saturated = calibrated;
        if (airSpread_ > 0.0f) {
            const double dryFaded = static_cast<double>(raw.dryBody) * fade;
            const double dry = Sd606ClapDetail::kSaturationThreshold * std::tanh(
                dryFaded / Sd606ClapDetail::kSaturationThreshold);
            ...
            const double lateWeight = Sd606ClapDetail::raisedCosineRamp(
                time, 0.045, 0.170);
            ...
            const double denseNoise = Sd606ClapDetail::kAirDensityThreshold
                * std::tanh(2.0 * calibratedNoise
                    / Sd606ClapDetail::kAirDensityThreshold);
        ...
        const float output = flushDenormal(static_cast<float>(
            saturated * Sd606ClapDetail::kOutputTrim));
...
private:
    double sampleRate_ = Sd606ClapDetail::kReferenceSampleRate;
    bool active_ = false;
    ...
    Sd606ClapDetail::ClapCore core_;
    Sd606ClapDetail::CoreUpsampler upsampler_;
};

} // namespace SynthDrums606

#endif /* SD606_CLAP_VOICE_H */
