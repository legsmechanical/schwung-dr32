#pragma once
// ============================================================================
//  airwin_spaces.h - Airwindows *Verbity2*, DR32's flexible "Spaces" reverb.
//
//  Original algorithm (c) Chris Johnson / Airwindows, MIT licence. Transplanted
//  VERBATIM by tools/port_airwindows.py; only the VST base class is replaced.
//  See vendor/SOURCES.md.
//
//  ⚠ THIS ONE DOES NOT NULL AGAINST UPSTREAM, AND THAT IS CORRECT. Unlike the
//  other transplants (-141 to -142 dB), Verbity2 nulls at only about -42 dB --
//  because it uses the `fpd` random state as an AUDIBLE modulator, not just as
//  dither:
//
//      double interpolateL = interpolateMax + (interpolateMax * (double(fpdL)/UINT32_MAX));
//
//  fpd is seeded from rand() in reset(), so every instance randomises its
//  delay-line interpolation differently. Measured control, so nobody re-chases
//  this: UPSTREAM against ITSELF (two instances, different seeds) also measures
//  -42.60 dB, port against upstream measures -42.54 dB, and port against port
//  with the fpd seed forced equal measures -342.33 dB, i.e. bit-exact. The
//  transplant is faithful; the plugin is simply non-deterministic by design.
//
//  Why one flexible model instead of a Room and a Hall: measured across the
//  whole Airwindows catalogue, the dedicated candidates each failed something.
//  Chamber (what DR32 shipped) ran as two independent MONO reverbs and its
//  tails were brighter than a plate. kGuitarHall2 measured -22.9 dB/Hz, far
//  darker than any real plate, and could not be brightened from its own
//  controls -- sweeping its early-reflection level moved the tail 0.00 dB.
//  kRockstar and kCosmos come from the infinite/ambient family. PocketVerbs'
//  six types do not span usefully (its "Room" produces no tail at all, Spring
//  and Tiled cap at 0.71 s) and it costs 190-581 ns/sample.
//
//  Verbity2 covers the whole span from one control set, at the LOWEST cost of
//  anything measured (~53 ns/sample):
//
//    A RmSize  <- Size    0.11-4.75 s at size .30 ... 0.37-16.1 s at size .75,
//                         and the onset grows with it (9.8 -> 32.8 ms), which
//                         is what a larger space physically does.
//    B Sustain <- Decay   scaled: past 0.6 raw it runs to 30-100 s.
//    C Mulch   <- Damp    a true tone control: density -0.26 -> -14.69 dB/Hz
//                         while RT60 stays at 2.48-2.51 s.
//
//  It is MONO by design (L/R correlation ~0.99). That was a disqualifier for
//  the earlier search and is deliberately not one here.
//
//  Known trade: it grows SPARSER with size (early crest 16.0 at size .30 up to
//  23.7 at .70), where the dedicated Dattorro plate holds 12.9. That density is
//  what makes a plate sound like a plate, which is why DR32 keeps its own plate
//  rather than asking this to do that job too.
// ============================================================================
#include <cmath>
#include <cstdlib>
#include <cstdint>
...
// PERFORMANCE NOTES, both confined to the dither (~157 dB down):
//  1. `5.5e-36l` -> `5.5e-36`. The `l` suffix makes it LONG DOUBLE, which on
//     aarch64 is IEEE binary128 emulated in SOFTWARE -- measured at ~1.2% of a
//     Move core per stage. Invisible on x86/Apple Silicon, where it is hardware.
//  2. `pow(2, expon+62)` -> `ldexp(1.0, expon+62)`, bit-identical for integer
//     exponents. Both verified still null against upstream.

// ---------------------------------------------------------------------------
//  Verbity2 -- (c) Chris Johnson / Airwindows, MIT licence.
...
// ---------------------------------------------------------------------------
namespace awk_verbity2 {

struct Verbity2 {
    // upstream parameter defaults
    float A = 0.5; float B = 0.5; float C = 0.5; float D = 1.0;
