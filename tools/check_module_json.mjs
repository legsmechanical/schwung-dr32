...
//
//    ⭑ THE PAD KNOBS ARE FOUR SIBLING LEVELS (Sample / Shape / Mix / Punch), one
//    per bank, sharing one focus. (Punch left Mix for Wide, 2026-09-22.) They must agree about the children they address,
//    or two banks are editing two different pads while showing one pad number.
const PAD_LEVELS = ['pads', 'pad_shape', 'pad_mix', 'pad_wide_ab', 'pad_punch'];
...
// 5a. the engine gate — ui_engine — and the pages that hang off it.
{
    ...
    for (const [lname, what] of [['pad_shape', 'Shape'], ['pad_punch', 'Punch']])
        if (!onGate(levels[lname] && levels[lname].visible_if, 0))
            errors.push(`levels.${lname} must be gated visible_if ${GATE} == 0 — ${what} is the sample voice's`);
    for (const [lname, keys] of [['pads', ['start', 'end']]]) {
        for (const k of keys) {
